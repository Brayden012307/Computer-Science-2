...
class Book:
    ...
    def progress(self):
        ...
        percent = (self._current_page / self.total_pages) * 100 
        return f"{percent:.1f}%" 
...
class Workout:
    ...
    def __str__(self):
        return f"{self.name}\n{self.duration_minutes}min on {self.date} ({self.calories_burned():.0f} cal)" 
...
if __name__ == "__main__":
    ...
    log.add_workout(run); log.add_workout(lift); log.add_workout(bike)
    log.summary()
